...
class MacroOrm
  ...
  macro map(mapping, strict = true)
    ...
    def self.all(*args) : Array({{ type_name }})
      ret = [] of {{ type_name }}
      {% if debug %}print "{{type_name}}.all "; %enterTime = Time.now{% end %}
      if args.empty?
      ...
      {% if debug %}\
        puts "#{ ((Time.now - %enterTime).nanoseconds.to_f/1000000).to_s.colorize :green }ms: #{ret.size} #{ args }"
      {% end %}\
      ret
    end

    def self.one?(*args) : {{ type_name }}?
      {% if debug %}print "{{type_name}}.one? "; %enterTime = Time.now{% end %}
      ret = if args.empty?
      ...
      {% if debug %}\
        puts "#{ ((Time.now - %enterTime).nanoseconds.to_f/1000000).to_s.colorize :green }ms: #{ ret ? "1" : "0" } #{ args }"
      {% end %}\
      ret
    rescue e
    ...
    {% if primaryName %}\
      ...
      def insert : {{primaryType}}
        {% if debug %}print "{{type_name}}.insert "; %enterTime = Time.now{% end %}
        ...
        {% if debug %}\
          puts "#{ (Time.now - %enterTime).nanoseconds.to_f/1000000 }ms: #{ @{{primaryName}} }"
        {% end %}\
      ...
      def insert_with_id
        ...
        {% if debug %}print "{{type_name}}.insert_with_id "; %enterTime = Time.now{% end %} 
      ...
      def update
        {% if debug %}print "{{type_name}}.update "; %enterTime = Time.now{% end %} 
    ...
    {% else %}\
      def insert : Nil
        ...
        {% if debug %}\
          puts "#{ (Time.now - %enterTime).nanoseconds.to_f/1000000 }ms"
        {% end %}\
      end
    {% end %}\
...
spawn do
  begin 
    nextUp = Time.now # just now
    loop do
      if (timeCurrent = Time.now) >= nextUp
